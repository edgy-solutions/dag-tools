...
- **`synthetic_classes_with_probe_coverage`** counts classes whose probes PASSED on BOTH baseline AND candidate. One-sided passes don't count. Removes the missing-coverage blocker for that class.
- **`synthetic_classes_red`** lists classes whose probe ran-and-FAILED on either side. This is a real regression signal and blocks GO **regardless** of `--accept-synthetic-coverage-missing` — that flag excuses missing coverage, NOT actively-failing probes. Don't add an opt-out for synthetic_classes_red. Test `test_verdict_no_go_when_probe_failed_even_with_synthetic_accept` enforces this.
