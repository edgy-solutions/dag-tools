...
- **The runner launches ONLY the downstream asset.** `<module_name>_downstream` has `deps=[<module_name>_upstream]`, so Dagster materializes the upstream automatically as part of the same run — and the downstream's identity assertion is what tells us the IO manager round-trip succeeded. Launching the pair separately would split a single semantic probe into two run records.
