...
### 4g1. Synthetic probes (`dag_tools.qual.synthetic`)
...
- **Generation only in v1.** `qual synthetic` produces sources + manifest; it does NOT deploy, run, or feed coverage into Q6. `qual report` still requires `--accept-synthetic-coverage-missing` to GO past synthetic classes. Don't paper this over — the operator deploying the location is the integration point that lets them choose their staging-config wiring.
