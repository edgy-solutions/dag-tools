"""Mesh-aware demo component (relocated from pub-tools).
...
**History**: this component originally lived inside ``pub_tools``
because pub-tools was the first user-deployment to land in the mesh
and the demo content followed it there for convenience. That muddled
pub-tools' purpose (which is to be the first REAL data-plane query
example, not a demo container), so this component moved here under
...
class MeshDemoComponent(Component, Resolvable, Model):
    ...
    and the S3 file name. **Do not change** without coordinating a
    URN migration — the existing DataHub entry and prior
    materialization history are keyed on this exact name."""
